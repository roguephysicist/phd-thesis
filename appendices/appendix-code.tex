...
With the help of Eq.~\eqref{chon.98}, we rewrite Eq.~\eqref{nmes}  as
\begin{align}\label{ccu51}
(v^{\lda,\rma}_{nm})_{;k^{\rmb}}&= 
i  
r^{\rmb}_{nm}
\Delta^{\rma}_{mn}
...
,
\end{align}  
where $\calt^{\rma\rmb}_{nm}$ is neglected.
We generalize this expression for the layer-by-layer scheme as
follows,
\begin{align}\label{ccu52}
...
,
\end{align} 
which is coded instead of Eq.~\eqref{c-a.2nn}. Likewise, with the help
...
,
\end{align}
which is coded instead of Eq.~\eqref{dgvs.cv}.
