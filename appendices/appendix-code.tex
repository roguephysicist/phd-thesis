...
transparent and straight forward. This coding is what constitutes the
...
is if it were a surface (even though they make no sense).
...
\item compile in \verb=$TINIBA/latm/SRC_1setinput=
\item run \verb=all_responses.sh= using\\ 
\verb=-w layer -r 44 ...=\\
\verb=-w total -r 21 ...=\\
and \\
\verb=-w total -r 42 ...=\\
thus obtaining a layered calculation using bulk matrix elements, a
bulk calculation for the length  and the velocity gauge, and plot the
three $\chi$'s, they ouught to be identical, if not CRY!.  
\end{enumerate}
