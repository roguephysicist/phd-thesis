...
The formulas we use for the calculation of the second order
susceptibilities
were obtained within the length gauge scheme, and are
such that the scissors operator is included. On the other hand, the
formulas used by Guo and Lin, were derived in Ref. [2] within the
velocity gauge scheme, where the scissor operator was not introduced
(chequen esto). Indeed, the correct way of introducing the effect of
the scissors operator, within the velocity gauge, was done in
Ref. [1], whereupon it was proved that both gauges satisfy gauge
invariance, as they must, but only after the scissors operator is
treated correctly. We remark that if the scissors correction is
neglected, as done by Guo and Lin, then our formulas and the ones used
by them are equivalent, and give the same results as shown in Ref. [1].
