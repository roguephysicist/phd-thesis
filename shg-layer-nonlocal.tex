...
\mathrm{Im}[\chi_{e,\rma\rmb\rmc,\go}^{s,\ell}] =
-\frac{\pi |e|^3}{2\hbar^2}\sum_{vc\bfk}\sum_{l\neq(v,c)}\frac{1}{(\omega^{S}_{cv})^{2}}
\left[
\frac{\go^S_{lc}\mathrm{Im}[\mathcal{V}^{\Sigma,\text{a}}_{lc}\{r^{\rmb}_{cv}r^{\rmc}_{vl}\}]}{(2\go^S_{cv}-\go^S_{cl})}
-\frac{\go^S_{vl}\mathrm{Im}[\mathcal{V}^{\Sigma,\text{a}}_{vl}\{r^{\rmc}_{lc}r^{\rmb}_{cv}\}]}{(2\go^S_{cv}-\go^S_{lv})}
