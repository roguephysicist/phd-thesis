...
\include{appendix-vnl}%appvnl B
\section{\texorpdfstring{$V^{\gs,\rma,\ell}_{nm}$}{Vnm} and \texorpdfstring{$\Big({\cal V}^{\gs,\rma,\ell}_{nm}\Big)_{;k^\rmb}$}{(Vnm);kb} }\label{calvs}
\include{appendix-gen_der_calv}%calvs C
\section{Generalized derivative \texorpdfstring{$(\go_n(\bfk))_{;\bfk}$}{(wn);k}}\label{gwk}
\include{appendix-gen_der_omega}%gwk D
\section{Expressions for \texorpdfstring{$\chi^S_{\rma\rmb\rmc}$}{Xabc} in terms of \texorpdfstring{$\mathcal{V}^{\gs,\text{a},\ell}_{mn}$}{Vmn}}
\label{appv}
\include{appendix-calv}%appv E
\section{Matrix elements of \texorpdfstring{$\tau^{\rma\rmb}_{nm}(\bfk)$}{tnm}}\label{calt}
\include{appendix-tnm}%calt F
\section{Explicit expressions for \texorpdfstring{${\cal V}^{a,\ell}_{nm}(\bfk)$ and ${\cal C}^{\ell}_{nm}(\bfk)$}{Vnm and Cnm}}\label{calpcalc}
\include{appendix-vc}%calpcalc G
\section{Generalized derivative \texorpdfstring{$(\bfr_{nm}(\bfk))_{;\bfk}$} {{rnm};k} for non-local potentials}\label{gdernl}
\include{appendix-gen_der_r_nonlocal}%gdernl H
\section{Coding}\label{code}
\include{appendix-code}%code I
\section{Divergence Free Expressions for $\chi^s_{\rma\rmb\rmc}$}
\include{appendix-div_free_exp_chi}
\section{Some results of Dirac's notation}\label{ap_dirac}
\include{appendix-dirac}
\section{Basic relationships}\label{ap_basic}
\include{appendix-basic_relationships}
\section{Generalized derivative $(\bfr_{nm}(\bfk))_{;\bfk}$}\label{gder}
\include{appendix-gen_der_r}
\section{$\Big({\cal R}^\rma_{nm}\Big)_{;k^\rmb}$}\label{calr}
\include{appendix-gen_der_calr}
\section{Odds and Ends}
\include{appendix-odds_ends}
